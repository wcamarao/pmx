...
func (s *UpdateSuite) TestAutoGenerated() {
	row := s.conn.QueryRow(
		...
		"update-auto-generated-old-label",
	)
	...
	event := fxt.Event{
		...
		Label:    "update-auto-generated-new-label",
	}
	...
	var position int64
	var label string
	row = s.conn.QueryRow(context.Background(), "select * from events where label = $1", "update-auto-generated-new-label")
	err = row.Scan(&position, &label)
	s.Equal(insertedPosition, position)
	s.Equal("update-auto-generated-new-label", label)
	s.Nil(err)
}
