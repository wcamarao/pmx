...
func (s *InsertSuite) TestAutoGenerated() {
	event := fxt.Event{
		Label: "insert-auto-generated-label",
	}
	...
	var position int
	var label string
	row := s.conn.QueryRow(context.Background(), "select * from events where label = $1", "insert-auto-generated-label")
	err = row.Scan(&position, &label)
	s.Equal("insert-auto-generated-label", label)
	s.NotZero(position)
	s.Nil(err)
}
